...
obj hit(in vec3 pos) {//obj     pos                     size                    material    smoothness
    obj o = Sphere( pos - vec3(0,64.5,0),    50,                      3);
        ...
        switch(o.type) {
            ...
            case 3: // white fog
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    if (curr.depth < 0) {
                        vec3 samplerPos = posInside * 0.2;
                        vec2 playerDirection = normalize(vec2(rd.x, rd.z));
                        samplerPos.y -= GameTime * 300;
                        // samplerPos += vec3(playerDirection.x, 0, playerDirection.y) * GameTime * 300;
                        vec2 noise = vec2(1.0) - worley(samplerPos, 1.0, false);
                        float noiseVal = (noise.y - noise.x + 1.3) * 2;
                        float colorVal = noiseVal * 0.069;
                        color.a += colorVal;
                    }
                    posInside += rd / .26;
                }
